	...
	def __init__(self,root):
		...
		def add_data():
			if self.roll_no.get()=="" or self.name.get()==""  or self.father_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				...
				con=pymysql.connect(host='localhost',user='root',password='Harsh@2002',database='student_management_system')
				cur=con.cursor()
...
def registration():
    conn1=custom("localhost","root","Harsh@2002","verified_no")
    mob=e_mob.get()
    name=e_name.get()
    pwd=e_pwd.get()
    email=e_mail.get()
...
def show():
    mobile=e1.get()
    password=e2.get()
    conn=custom('localhost', 'root', 'Harsh@2002','verified_no')
...
img1=Image.open(r"C:\Users\HARSH GUPTA\Desktop\final project 2\Login pAGE.png")
